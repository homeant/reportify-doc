...
`market` 可选 `cn`/`hk`/`us`/`global`（默认 `cn`）。获取指数行情（`stockType=index`）时，除 A 股（`cn`）、港股（`hk`）、美股（`us`）外，其余指数统一通过 `global` 查询。
...
<Note>
目前仅支持 **A 股**（`market: 'cn'`）。港股、美股等暂未支持，传入会抛出错误。
</Note>
...
```typescript
const days = await client.quant.calendar({
  startDate: '2026-01-01',
  endDate: '2026-01-31'
});

console.log(days.map((d) => d.date));
// ['2026-01-05', '2026-01-06', ...]
```

| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `startDate` | `string` | 否 | 开始日期 `YYYY-MM-DD`，默认当年 1 月 1 日 |
| `endDate` | `string` | 否 | 结束日期 `YYYY-MM-DD`，默认当年 12 月 31 日 |
| `market` | `string` | 否 | 市场，默认 `cn`（仅支持 A 股） |
| `exchange` | `string` | 否 | 交易所 `sh`/`sz`，默认市场对应交易所（A 股为 `sh`） |
...
```typescript
const info = await client.quant.calendarCheck({ target: '2026-01-01' });

console.log(info?.is_trading_day);        // false
console.log(info?.previous_trading_day);  // '2025-12-31'
console.log(info?.next_trading_day);      // '2026-01-05'
```

| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `target` | `string` | 否 | 待判断日期 `YYYY-MM-DD`，默认今天 |
| `market` | `string` | 否 | 市场，默认 `cn`（仅支持 A 股） |
| `exchange` | `string` | 否 | 交易所 `sh`/`sz`，默认市场对应交易所（A 股为 `sh`） |

返回字段：`date`、`is_trading_day`、`previous_trading_day`、`next_trading_day`。
