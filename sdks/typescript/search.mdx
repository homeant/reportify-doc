...
| `options.categories` | `string[]` | 否 | - | 文档类别过滤，可选值：`news`, `reports`（国内研报）, `global_research`（外资研报）, `filings`, `transcripts`, `socials`。 |
...
| `query` | `string` | 是 | - | 搜索关键词 |
| `options.num` | `number` | 否 | `10` | 返回结果数量 |
| `options.categories` | `string[]` | 否 | `['reports', 'global_research']` | 研报类别，可选值：`reports`（国内研报）、`global_research`（外资研报）。默认两者全选，可单独指定其一以缩小范围 |
| `options.symbols` | `string[]` | 否 | - | 股票代码过滤 |
| `options.industries` | `string[]` | 否 | - | 行业过滤 |
| `options.channelIds` | `string[]` | 否 | - | 渠道 ID 过滤 |
| `options.startDatetime` | `string` | 否 | - | 开始时间 |
...
interface Document {
  ...
  title: string;
  summary: string;
  category: 'news' | 'reports' | 'global_research' | 'filings' | 'transcripts' | 'socials';
