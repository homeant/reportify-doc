...
|------|------|------|------|
| `symbol` | `str` | 是 | 股票代码 |
| `market` | `str` | 否 | 市场，默认 cn，可选 `cn`/`hk`/`us`/`global`。获取指数行情（`stock_type=index`）时，除 A 股（`cn`）、港股（`hk`）、美股（`us`）外，其余指数统一通过 `global` 查询 |
...
获取指定区间内的 A 股交易日列表，返回带 `date` 列的 DataFrame（按日期升序）。

```python
df = client.quant.calendar(
...
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `start_date` | `str` | 否 | 开始日期 `YYYY-MM-DD`，默认当年 1 月 1 日 |
| `end_date` | `str` | 否 | 结束日期 `YYYY-MM-DD`，默认当年 12 月 31 日 |
| `market` | `str` | 否 | 市场，默认 `cn`（仅支持 A 股） |
| `exchange` | `str` | 否 | 交易所 `sh`/`sz`，默认市场对应交易所（A 股为 `sh`） |
...
```python
info = client.quant.calendar_check("2026-01-01")

print(info["is_trading_day"])        # False
print(info["previous_trading_day"])  # '2025-12-31'
print(info["next_trading_day"])      # '2026-01-05'
```

| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `target` | `str` | 否 | 待判断日期 `YYYY-MM-DD`，默认今天 |
| `market` | `str` | 否 | 市场，默认 `cn`（仅支持 A 股） |
| `exchange` | `str` | 否 | 交易所 `sh`/`sz`，默认市场对应交易所（A 股为 `sh`） |
